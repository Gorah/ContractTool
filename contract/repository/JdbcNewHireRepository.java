...
public class JdbcNewHireRepository implements NewHire {
	...
	public JdbcNewHireRepository(DataSource dataSource, ComboOptions opts) throws SQLException{
		this.conn = dataSource.getConnection();
		this.opts = opts;
		INSERT_EMPLOYEE = conn.prepareStatement("INSERT INTO Hires VALUES (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
				+"?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
	...
	public long add(HireAbstractModel hire) throws EntityAlreadyExistsException {
		...
		try {
			...
			if(hire.getDetail("eeid").isEmpty()){
				INSERT_EMPLOYEE.setNull(2, Types.NULL);
			} else {
			...
			if(hire.getDetail("address_line2").isEmpty()){
				INSERT_EMPLOYEE.setNull(6, Types.NULL);
			} else {
				INSERT_EMPLOYEE.setString(6, hire.getDetail("addressLine2"));
			}
			...
			if(hire.getDetail("signatory_name").isEmpty()){
				INSERT_EMPLOYEE.setNull(13, Types.NULL);
			} else {
			...
			if(!hire.getDetail("date_tbc").toLowerCase().equals("true")){
				try {
					...
					INSERT_EMPLOYEE.setNull(27, Types.NULL);
				}
			} else {
				INSERT_EMPLOYEE.setNull(27, Types.NULL);
			}
			if(!hire.getDetail("end_date").isEmpty()){
				try {
					...
					INSERT_EMPLOYEE.setNull(28, Types.NULL);
			...
			if(!hire.getDetail("probtion_duration").isEmpty()){
				INSERT_EMPLOYEE.setInt(32, Integer.parseInt(hire.getDetail("probation_duration")));
			} else {
				INSERT_EMPLOYEE.setNull(32, Types.NULL);
			}
			...
			if(!hire.getDetail("sign_on_bonus_amount").isEmpty()){
				...
				INSERT_EMPLOYEE.setNull(34, Types.NULL);
			}
			...
			if(!hire.getDetail("travel_supplement_start").isEmpty()){
				try {
					...
					INSERT_EMPLOYEE.setNull(37, Types.NULL);
			...
			if(!hire.getDetail("travel_supplement_duration").isEmpty()){
				...
				INSERT_EMPLOYEE.setNull(38, Types.NULL);
			}
			...
			if(!hire.getDetail("mcbc_sharps").isEmpty()){
				...
				INSERT_EMPLOYEE.setNull(48, Types.NULL);
			}
			try {
			...
			if(!hire.getDetail("reason_for_contract").isEmpty()){
				...
				INSERT_EMPLOYEE.setNull(51, Types.NULL);
			}
